...
class HybridDecryption:
    ...
    def decrypt_rsa_only(
        self, filename="rsa_encrypted.json", privkey_path="keys/private_key.pkl"
    ):
...
def main():
    ...
    while True:
        ...
        choice = input("\nmake your choice nowww (1-6): ").strip()
        ...
        if choice == "1":
            ...
            # Decrypt
            decrypted_message = crypto.hybrid_decrypt(
                encrypted_message, encrypted_aes_key, privkey
            )
            ...
            try:
                ...
                encrypted_file = input(
                    "Enter encrypted data file path (default: encrypted_data.json): "
                ).strip()
                if not encrypted_file:
                    encrypted_file = "encrypted_data.json"

                privkey_file = input(
                    "Enter private key file path (default: keys/private_key.pkl): "
                ).strip()
                if not privkey_file:
